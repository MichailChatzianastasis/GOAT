...
from layers import GOATlayer, GraphAttentionLayer,LSTMAggregation,GraphAttentionV2LayerOrdered, GATIMP4layer, GraphAttentionLayerRandomOrdered, GraphAttentionLayerOurs
import os
from time import ctime
from torch_geometric.nn import GCNConv,SAGEConv,GATv2Conv, GATConv,GINConv,PNAConv
import pickle
import itertools
...
class GOAT(nn.Module):
    def __init__(self, nfeat, nhid, nhid_2, nclass, lstm_h1, lstm_h2, pooling_1, pooling_2, dropout, alpha, nheads=8, nheads_2=1, dataset="cora", rnn_agg="lstm"):
        ...
        #second layer
        self.out_att = [GOATlayer(in_features=self.output_1, out_features=nhid_2, lstm_out_features=lstm_h2, dropout=dropout, alpha=alpha, concat=False, dataset=dataset) for _ in range(nheads_2)]
        #self.out_att = [GraphAttentionLayer(self.output_1, lstm_h2, dropout=dropout, alpha=alpha, concat=True, dataset=dataset) for _ in range(nheads_2)]
    ...
    def forward(self, x, adj):
        ...
        #second layer
        if(self.pooling_2 == "cat"):
            x = torch.cat([out_att(x, adj,epoch=self.epoch) for out_att in self.out_att], dim=1)
        else:
            x = torch.sum(torch.stack([out_att(x, adj,epoch=self.epoch) for out_att in self.out_att]),dim=0)
...
class GAT(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.out_att(x, adj,self.epoch)
        return F.log_softmax(x, dim=1)
